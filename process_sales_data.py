...
def get_sales_csv_path():
    ...
    if not os.path.isfile(csv_path):
        print("Error: CSV path is not an exsisting file.")
        sys.exit()
...
def create_orders_dir(sales_csv_path):
    ...
    """
    # Getdirectory in which sales data CSV file resides
    sales_cvs_dir = os.path.dirname(sales_csv_path)

    # Determine the path of the directory to hold the order data files
    todays_date = date.today().isoformat()
    orders_dir = f'Orders_{todays_date}'
    orders_dir_path = os.path.join (sales_cvs_dir,orders_dir)
